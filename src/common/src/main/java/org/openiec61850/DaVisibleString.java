...
public class DaVisibleString extends BasicDataAttribute {

	private byte[] value;
	private int maxLength;

	public DaVisibleString(ObjectReference objectReference, FunctionalConstraint fc, String sAddr, byte[] value,
		...
		this.fc = fc;
		this.basicType = DaType.VISIBLE_STRING;
		this.sAddr = sAddr;
		this.value = value;
		this.dchg = dchg;
		this.dupd = dupd;
	...
	public void setValue(byte[] value) {
		if (value.length > maxLength) {
			throw new IllegalArgumentException("VISIBLE_STRING value size exceeds maxLength of " + maxLength);
		}
		this.value = value;
	}
	...
	@Override
	public DaVisibleString copy() {
		return new DaVisibleString(objectReference, fc, sAddr, value, maxLength, dchg, dupd);
	}
